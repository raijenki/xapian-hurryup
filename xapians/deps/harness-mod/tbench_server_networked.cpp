...
size_t NetworkedServer::recvReq(int id, void** data) {
    ...
    //RN TODO: Add this to a temporary buffer. 
    
    if (clientFds.size() == 0) {
        std::cerr << "All clients exited. Server finishing" << std::endl;
        /*
         * Bad way to exit, we aren't finishing what we need to after.
         */
        exit(0);
    } else {
    ...
    //std::cout << "Queue length " << this_id << " " << qlen << std::endl;
    //std::cout << "receieve req " << req->id << std::endl;
    return req->len;
};
...
void NetworkedServer::sendResp(int id, const void* data, size_t len) {
    ...
    // RN:  Pop from queue
    recvReq_queue.pop(); 
    //std::cout << "Send response " << resp->id << std::endl;
    uint64_t curNs = getCurNs();
    ...
    //RN Slightly redudant to do if before assert, but it's the easiest way to print stuff.
    if (sent != totalLen) {
        std::cout << sent << " " << totalLen << std::endl;
...
size_t NetworkedServer::Queuelen() {
    return recvReq_queue.size();
}
...
size_t tBenchQueuelen(void** data) {
    return server->Queuelen();
}
