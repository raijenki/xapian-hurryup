...
def buildDeadlineModel(a, b):
  model = keras.Sequential([
          layers.Dense(64, activation='relu', input_shape=[len(a.keys())]),
  ...
  final = model.fit(a, b, epochs=1000, validation_split=0.2, callbacks=[early_stop])
  return final

def buildEnergyModel(a, b):
  model = keras.Sequential([
          layers.Dense(64, activation='relu', input_shape=[len(a.keys())]),
          layers.Dense(16, activation='relu'),
          layers.Dense(4, activation='relu'),
...
def energyData(db):
    ...
    train_dataset = db.sample(frac=0.7,random_state=0)
    train_stats = train_dataset.describe()
    train_stats.pop('energyconsumption')
    train_stats = train_stats.transpose()
    train_labels = train_dataset.pop('energyconsumption')
