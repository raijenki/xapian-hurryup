...
do
	...
	do
	cat /sys/devices/system/cpu/cpu0/cpufreq/stats/time_in_state > /home/cc/hurryup-dvfs/raw-results/time_in_state/tis-hup-gov@ondem-${counter}-${queryPerSecond}-start.txt
	...
	cat /sys/devices/system/cpu/cpu0/cpufreq/stats/time_in_state > /home/cc/hurryup-dvfs/raw-results/time_in_state/tis-hup-gov@ondem-${counter}-${queryPerSecond}-end.txt
	consumption=$((energyEnd - energyStart))
	echo "$counter, $queryPerSecond, $consumption" >> energy-hup.txt
	sudo python parselats.py lats.bin
	mv lats.txt ../../raw-results/time_on_state/hup-gov@ondem-${counter}-${queryPerSecond}.bin
	rm server.pid
	done
	mv energy-hup.txt ../../raw-results/time_on_state/energy-hup-gov@ondem-${queryPerSecond}.txt
done
