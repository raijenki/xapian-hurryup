...
void emeter_init(struct emeter *this, char *outfile)
{
	...
	if(this->out) {
		fprintf(this->out, "mslong,sys_current,a57_current,a53_current,gpu_current,"
...
void emeter_take_reading(struct emeter *this)
{
	...
	unsigned long long mslong = (unsigned long long) tp.tv_sec * 1000 + tp.tv_usec / 1000;

	static struct reading reading;
	int error_count = 0;
	emeter_read_measurements(this, &reading);
	int ret = fprintf(this->out, "%llu,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",
			mslong,
